...
class ToolRepository @Inject constructor(
    ...
    suspend fun syncTools(): Flow<Result<List<Tool>>> = flow {
        try {
    ...
    suspend fun returnTool(checkoutId: Int, returnRequest: ReturnRequest): Flow<Result<ToolCheckout>> = flow {
        try {
            val response = apiService.returnTool(checkoutId, returnRequest)
            if (response.isSuccessful) {
                val returnedCheckout = response.body()!!
                toolCheckoutDao.updateCheckout(returnedCheckout)
                
                // Update tool status
                val checkout = toolCheckoutDao.getCheckoutById(checkoutId)
                checkout?.let {
